...
class SearchInterface:
    def __init__(self, search_engine):
        ...
        self.int_slider = widgets.IntSlider(
            value=10, min=1, max=20, step=1, description="Résultats"
        )
        self.button = widgets.Button(description="Rechercher")
        self.output = widgets.Output()
        self.interface = widgets.VBox(
            [self.label, self.text_input, self.int_slider, self.button, self.output]
        )
    ...
    def click_button(self, _):
        with self.output:
            ...
            if keywords:
                ...
                results = self.search_engine.search(keywords, top_n)

                if not results.empty:
                    print("\nRésultats de la recherche :")
                    display(results[["Document ID", "Score", "Texte"]])
                else:
